...
function SquareImage({ src, title, opacity }) {
  ...
  return (
    <div className={`relative flex w-full h-full rounded-xl dark:opacity-50 opacity-85 transition-all duration-300 hover:opacity-100 cursor-pointer group`} title={title}>
      {loading && 
      ...
      <img src={src} className='h-full w-full object-cover object-top rounded-xl group-hover:scale-[1.03] transition' onLoad={handleLoad} />
    </div>
  )
}
